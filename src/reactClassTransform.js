...
module.exports = (file, api, options) => {
    ...
    case 'CallExpression': { // type class
      ...
      case 'shape': {
        ...
        rawPropList.forEach(typeProp => {
          ...
          const [valueType, optional] = propTypeToFlowAnnotation(typeProp.value);
  ...
  const createESClass = (
    ...
    const maybeFlowStateAnnotation = []; // we only need this when we do `this.state = ...`
  ...
  if (
    ...
    const removeRequire = (pathAndBinding) => {
      if (!pathAndBinding) {
        return;
      }

      const { binding, path, type } = pathAndBinding;
    ...
    const createClassPath = ReactUtils.findAllReactCreateClassCalls(root);
    if (NO_CONVERSION) {
      createClassPath.forEach(childPath => {
        fallbackToCreateClassModule(childPath);
      });
      didFallback = true;
    } else {
      // the only time that we can't simply replace the createClass call createClassPath
      // with a new class is when the parent of that is a variable declaration.
      // let's delay it and figure it out later (by looking at `createClassPath.parentPath`)
      // in `updateToClass`.
      createClassPath.forEach(childPath => {
        if (
    ...
    const createClassPathAndBinding = findRequirePathAndBinding(CREATE_CLASS_MODULE_NAME);
    ...
    if (didFallback) {
      ...
      if (reactPathAndBinding) {
        ...
        const shouldRemoveReactImport = (
          removePath &&
          root.find(j.Identifier).filter(p => p.value.name === 'React').length === 1 &&
    ...
    if (didTransform) {
      ...
      removeRequire(pureRenderMixinPathAndBinding);
      removeRequire(createClassPathAndBinding);
    }
  }
